...
\begin{itemize}
    ...
    Answer:
        ...
        \begin{tikzpicture}
            ...
            \node (prepare) [io, below of=start, yshift=-1cm, text width=5cm] {Prepare training set};
            \node (batch) [process, yshift=-1cm, below of=prepare] {Batchifying};
            \node (train) [decision, below of=batch, yshift=-2cm] {Training RNN};
            \node (hidden) [process, left of=train, xshift=-3cm, text width=2cm] {Hidden state};
            \node (loss) [process, below of=train, yshift=-2cm] {Compute Loss};
            
            % \draw [arrow] (start) -- (prepare)
            % \draw [arrow] (prepare) -- (batch)
            % \draw [arrow] (batch) -- (train)
        \end{tikzpicture}
